/*-----------------------------------------------
  ...
  内容：DS1302实时时钟数码管显示，时间和日期切换显示 按下按键，循环切换 
		...
		日期格式xx-xx-xx
		周秒格式-x-   xx
		开发板使用12M晶振，通过dofly自带的串口调试软件，打开串口，波特率默认2400，点击更新时间即可，如果不行，按下开发板复位重新更新
------------------------------------------------*/
...
unsigned char TempData[5]; //存储显示值的全局变量
...
if(displaynum==0) //显示时间
 ...
 TempData[0]=dofly_DuanMa[time_buf1[2]/10];//月
 TempData[1]=dofly_DuanMa[time_buf1[2]%10];
 TempData[2]=0x40;	//加入"-"
 TempData[3]=dofly_DuanMa[time_buf1[3]/10];//日
 TempData[4]=dofly_DuanMa[time_buf1[3]%10];	
 }
 ...
 TempData[3]=dofly_DuanMa[time_buf1[6]/10];//秒
 TempData[4]=dofly_DuanMa[time_buf1[6]%10];	
 }
 }	
 }
}
